### Hi, I'm Shirlei 👋

Solutions Architect working on data & analytics platforms — mostly SAP Datasphere for enterprise clients by day, and analytics engineering and applied AI on the side.

**What I work with**
- 🏗️ Data architecture & governance in the SAP ecosystem — Datasphere, BW/4HANA, HANA
- ☁️ Cloud data platforms and Agentic Engineering

**Some things I've built**
- [`case-impulsogov`](https://github.com/ShirleiAlexandrino/case-impulsogov) — an analytics engineering case for a healthcare data platform: a medallion pipeline (bronze/silver/gold) in DuckDB, LGPD-aware data minimization, and a Streamlit app with role-based access control.
  Live demo: [case-impulsogov.streamlit.app](https://case-impulsogov.streamlit.app/)
- `avalifit` — an AI-powered physical assessment platform for personal trainers.

**Education**
- Executive MBA, Arquitetura de Soluções — FIAP (2025–2026)
- MBA Executivo em Business Analytics e Big Data — Fundação Getulio Vargas (2017–2019)
- MBA em Gestão de Projetos — Ibmec (2014)
- MBA em Gestão de Processos — Fundação Getulio Vargas (2011–2013)
- Administração de Empresas — Universidade Federal Rural do Rio de Janeiro (1995–2000)

🏆 Featured among 19 professionals driving digital transformation in large corporations — [Forbes Insider, "Inovadores Negros"](https://forbes.com.br/forbes-tech/2020/09/especial-inovadores-negros-18-profissionais-que-estao-promovendo-a-transformacao-digital-em-grandes-corporacoes/)

📍 Rio de Janeiro, Brazil

🏃‍♀️ Outside of work: running, and continuously studying AI and Solutions Architecture

📫 [LinkedIn](https://linkedin.com/in/shirleialexandrino) · shirlei.alexandrino@gmail.com
